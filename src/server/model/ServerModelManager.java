...
//        PLEASE PUT THIS AFTER LOADING THE SCHEDULES AND APPOINTMENTS IF YOU WANNA MOVE IT
    ...
    @Override
    public synchronized void setRole(User user, String role) throws RemoteException {
        switch (role) {
            case "Nurse":
                try {
    ...
    @Override
    public synchronized void RemoveRole(User user) throws RemoteException {
    ...
    //wtf is this order
    //faq1 is local variable, list has to be changed from database after each change
    //changes to faq1 are useless
    @Override
